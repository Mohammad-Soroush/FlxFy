import React from 'react';
import { House, Todolist, User } from '../assetsJS';

function Footer() {

  return (
    <footer className="fixed bottom-4 mb-3 left-0 w-full bg-white border border-blue-500 rounded-full sm:rounded-2xl md:rounded-2xl lg:rounded-2xl py-3">
      <div className="container mx-auto max-w-screen-xl">
        <div className="footercollec flex justify-around items-center">
          <button
            className="home bg-blue-700 p-4 sm:p-5 rounded-full hover:bg-blue-600 active:bg-blue-400 transition duration-200">
            <img src={House} className="w-6 sm:w-8 md:w-9" alt="home" />
          </button>
          <div className="Todolist p-4">
            <img src={Todolist} className="w-8 pr-3 sm:w-8 md:w-9" alt="todolist" />
          </div>
          <div className="User p-3">
            <img src={User} className="w-6 sm:w-8 md:w-9" alt="user" />
          </div>
        </div>
      </div>
    </footer>
  );
}

export default Footer;